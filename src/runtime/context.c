...
#define VG_TYPE_SHIFT 60u
#define VG_CONTEXT_SHIFT 44u
#define VG_WORLD_INDEX_SHIFT 34u
#define VG_WORLD_GENERATION_SHIFT 22u
#define VG_ENTITY_INDEX_SHIFT 12u
#define VG_TYPE_MASK UINT64_C(0xF)
#define VG_CONTEXT_MASK UINT64_C(0xFFFF)
...
static void *vg_default_allocate(void *user, size_t size) {
    (void)user;
    return malloc(size);
}
...
uint64_t vg_runtime_make_world_handle(const VgContext *context, uint32_t world_index,
                                      uint16_t generation) {
    return ((uint64_t)VG_HANDLE_TYPE_WORLD << VG_TYPE_SHIFT) |
           ((uint64_t)context->tag << VG_CONTEXT_SHIFT) |
...
uint64_t vg_runtime_make_entity_handle(const VgContext *context, uint32_t world_index,
    ...
    return ((uint64_t)VG_HANDLE_TYPE_ENTITY << VG_TYPE_SHIFT) |
           ((uint64_t)context->tag << VG_CONTEXT_SHIFT) |
           ((uint64_t)(world_index + 1u) << VG_WORLD_INDEX_SHIFT) |
...
VgResult vg_runtime_resolve_world(VgContext *context, VgWorld handle, uint32_t *out_index,
    ...
    if (!vg_context_valid(context))
        return VG_ERROR_INVALID_ARGUMENT;
    if (handle.value == VG_INVALID_HANDLE_VALUE)
        return VG_ERROR_INVALID_HANDLE;
    if (vg_decode(handle.value, VG_TYPE_SHIFT, VG_TYPE_MASK) != VG_HANDLE_TYPE_WORLD)
        return VG_ERROR_WRONG_TYPE;
    if (vg_decode(handle.value, VG_CONTEXT_SHIFT, VG_CONTEXT_MASK) != context->tag)
        return VG_ERROR_WRONG_CONTEXT;
    uint32_t encoded_index = vg_decode(handle.value, VG_WORLD_INDEX_SHIFT, VG_INDEX_MASK);
    if (encoded_index == 0u || encoded_index > context->max_worlds)
        return VG_ERROR_INVALID_HANDLE;
    uint32_t index = encoded_index - 1u;
    VgWorldSlot *slot = &context->worlds[index];
    uint32_t generation = vg_decode(handle.value, VG_WORLD_GENERATION_SHIFT, VG_GENERATION_MASK);
...
VgResult vg_runtime_resolve_entity(VgContext *context, VgEntity handle, uint32_t *out_world_index,
    ...
    if (handle.value == VG_INVALID_HANDLE_VALUE)
        return VG_ERROR_INVALID_HANDLE;
    if (vg_decode(handle.value, VG_TYPE_SHIFT, VG_TYPE_MASK) != VG_HANDLE_TYPE_ENTITY)
        return VG_ERROR_WRONG_TYPE;
    if (vg_decode(handle.value, VG_CONTEXT_SHIFT, VG_CONTEXT_MASK) != context->tag)
        return VG_ERROR_WRONG_CONTEXT;
    uint32_t encoded_world = vg_decode(handle.value, VG_WORLD_INDEX_SHIFT, VG_INDEX_MASK);
    ...
    uint32_t world_generation =
        vg_decode(handle.value, VG_WORLD_GENERATION_SHIFT, VG_GENERATION_MASK);
    if (world_slot->state != VG_WORLD_ACTIVE || world_slot->generation != world_generation ||
        world_slot->world == NULL)
        return VG_ERROR_INVALID_HANDLE;
    uint32_t encoded_entity = vg_decode(handle.value, VG_ENTITY_INDEX_SHIFT, VG_INDEX_MASK);
